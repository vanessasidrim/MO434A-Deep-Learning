...
class SentimentFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        df = pd.concat(
            [df, df['headline_sents'].apply(lambda x: compute_sentiment(x))],
            axis=1)
        df.rename(columns={
            'compound': 'h_compound',
            'neg': 'h_neg',
        ...
        print('headline sentiment features of training set saved in %s' %
              outfilename_hsenti_train)
        ...
        if n_test > 0:
            ...
            print('headline sentiment features of test set saved in %s' %
                  outfilename_hsenti_test)
        ...
        df = pd.concat(
            [df, df['body_sents'].apply(lambda x: compute_sentiment(x))],
            axis=1)
        df.rename(columns={
            'compound': 'b_compound',
            'neg': 'b_neg',
        ...
        print('body sentiment features of training set saved in %s' %
              outfilename_bsenti_train)
        ...
        if n_test > 0:
            ...
            print('body sentiment features of test set saved in %s' %
                  outfilename_bsenti_test)
