...
def build_test_data():
    ...
    stances = pd.read_csv(
        "test_stances_unlabeled.csv")  # needs to contain pair id
    data = pd.merge(stances, body, how='left', on='Body ID')
...
def train():
    ...
    pred_prob_y = bst.predict(dtest).reshape(test_x.shape[0],
    ...
    stances = pd.read_csv(
        "test_stances_unlabeled_processed.csv")  # same row order as predicted
...
def show_incorrect_pred(actual, predicted, idx_valid):
    ...
    derr.to_csv(
        'incorrect.csv',
        columns=['Body ID', 'Stance', 'predicted', 'Headline', 'articleBody'],
        index=False)
