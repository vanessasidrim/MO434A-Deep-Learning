...
def load_data():
    ...
    yuxi = pd.read_csv('predtest_cor2.csv',
                       usecols=[
                           'Headline', 'Body ID', 'Stance', 'prob_0', 'prob_1',
                           'prob_2', 'prob_3'
                       ])
    print('yuxi.shape:')
    print(yuxi.shape)

    doug = pd.read_csv('dosiblOutput.csv',
                       usecols=[
                           'Headline', 'Body ID', 'Agree', 'Disagree',
                           'Discuss', 'Unrelated'
                       ])
    print('doug.shape:')
    print(doug.shape)
    ...
    x_meta = combine[[
        'prob_0', 'prob_1', 'prob_2', 'prob_3', 'Agree', 'Disagree', 'Discuss',
        'Unrelated'
    ]].values
...
def loadTest():
    ...
    yuxi = pd.read_csv('tree_pred_prob_cor2.csv',
                       usecols=[
                           'Headline', 'Body ID', 'prob_0', 'prob_1', 'prob_2',
                           'prob_3'
                       ])
    print('yuxi.shape:')
    print(yuxi.shape)
...
def stack_test():
    ...
    pred_agree = (x_meta[:, 0] * param['w0'] +
                  x_meta[:, 4] * param['w1']) / sumw
    pred_disagree = (x_meta[:, 1] * param['w0'] +
                     x_meta[:, 5] * param['w1']) / sumw
    pred_discuss = (x_meta[:, 2] * param['w0'] +
                    x_meta[:, 6] * param['w1']) / sumw
    pred_unrelated = (x_meta[:, 3] * param['w0'] +
                      x_meta[:, 7] * param['w1']) / sumw

    pred_y = np.hstack([
        pred_agree.reshape((-1, 1)),
        pred_disagree.reshape((-1, 1)),
        pred_discuss.reshape((-1, 1)),
