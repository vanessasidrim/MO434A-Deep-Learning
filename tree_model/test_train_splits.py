...
def split_it(df, percentage):
    ...
    return (df.key[a_indx].reset_index(drop=True),
            df.key[b_indx].reset_index(drop=True))
...
def break_it(df, percentage):
    ...
    while tagged_rows < target_size:
        ...
        this_indx = (df['Headline'] == this_headline) | (
            df['Body ID'] == this_body_id)
...
def find_all(df, this_indx):
    ...
    df_h_indx = [
        True if k in this_headlines.values else False for k in df['Headline']
    ]
    df_b_indx = [
        True if k in this_body_id.values else False for k in df['Body ID']
    ]
...
def get_repeats_df(df, groupby_cols):
    ...
    agg_functions = {stance: (lambda x: Counter(x)), body: 'count'}

    renaming = {stance: 'stances', body: 'counts'}

    grouped = df.groupby(groupby_cols, as_index=False)
    aggregated = grouped.agg(agg_functions).rename(
        columns=renaming).sort_values('counts', ascending=False)
    return aggregated
...
df.apply(lambda row: 'unrelated' if row[stance] == 'unrelated' else 'related',
         axis=1).value_counts()
...
if run_headline_body_split:
    headline_related_repeats = get_repeats_df(
        df_related, headline).rename(columns={headline: 'key'})
    body_related_repeats = get_repeats_df(
        df_related, body_id).rename(columns={body_id: 'key'})
    ...
    for ii in range(0, 5):
        ...
        df_test_h_indx = [
            True if k in test_h.values else False for k in df_related[headline]
        ]
        df_train_h_indx = [True if not a else False for a in df_test_h_indx]

        df_test_b_indx = [
            True if k in test_b.values else False for k in df_related[body_id]
        ]
        df_train_b_indx = [True if not a else False for a in df_test_b_indx]

        df_related[df_test_h_indx].to_csv('test_h_{}.csv'.format(ii),
                                          index=False,
                                          encoding='utf-8')
        df_related[df_train_h_indx].to_csv('train_h_{}.csv'.format(ii),
                                           index=False,
                                           encoding='utf-8')

        df_related[df_test_b_indx].to_csv('test_b_{}.csv'.format(ii),
                                          index=False,
                                          encoding='utf-8')
        df_related[df_train_b_indx].to_csv('train_b_{}.csv'.format(ii),
                                           index=False,
                                           encoding='utf-8')
...
if run_no_overlap_split:
    for ii in range(0, 5):
        ...
        df_test.to_csv('test_d_{}.csv'.format(ii),
                       index=False,
                       encoding='utf-8')
        df_train.to_csv('train_d_{}.csv'.format(ii),
                        index=False,
                        encoding='utf-8')
