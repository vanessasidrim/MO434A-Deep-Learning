...
class CountFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        for feat_name in feat_names:
            ...
            df['len_sent_%s' % feat_name] = df[feat_name].apply(
        ...
        print('train:')
        print(
            train[['Headline_unigram', 'Body ID',
        ...
        print('basic counting features for training saved in %s' %
              outfilename_bcf_train)

        test = df[df['target'].isnull()]
        print('test:')
        print(
            test[['Headline_unigram', 'Body ID', 'count_of_Headline_unigram']])
        #return 1
        if test.shape[0] > 0:
            # test set exists
            ...
            with open(outfilename_bcf_test, 'wb') as outfile:
                ...
                print('basic counting features for test saved in %s' %
                      outfilename_bcf_test)
