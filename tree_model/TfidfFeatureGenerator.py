...
class TfidfFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        vecH = TfidfVectorizer(ngram_range=(1, 3),
                               max_df=0.8,
                               min_df=2,
        ...
        print('headline tfidf features of training set saved in %s' %
              outfilename_htfidf_train)
        ...
        if n_test > 0:
            ...
            print('headline tfidf features of test set saved in %s' %
                  outfilename_htfidf_test)

        vecB = TfidfVectorizer(ngram_range=(1, 3),
                               max_df=0.8,
                               min_df=2,
                               vocabulary=vocabulary)
        ...
        print('body tfidf features of training set saved in %s' %
              outfilename_btfidf_train)
        ...
        if n_test > 0:
            ...
            print('body tfidf features of test set saved in %s' %
                  outfilename_btfidf_test)

        # 4). compute cosine similarity between headline tfidf features and body tfidf features
        simTfidf = np.asarray(map(cosine_sim, xHeadlineTfidf,
                                  xBodyTfidf))[:, np.newaxis]
        ...
        print('tfidf sim. features of training set saved in %s' %
              outfilename_simtfidf_train)
        ...
        if n_test > 0:
            ...
            print('tfidf sim. features of test set saved in %s' %
                  outfilename_simtfidf_test)
