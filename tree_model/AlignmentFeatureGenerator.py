...
class AlignmentFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        df['align_score_max'] = df.apply(lambda x: np.max([
            calc_hungarian_alignment_score(x['Headline'], bs, x.name)[1]
            for bs in x['body_sents']
        ]),
                                         axis=1)
        ...
        if n_test > 0:
            ...
            print('word alignment features for test saved in %s' %
                  outfilename_walign_test)
