...
class Word2VecFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        #headlineVec = [0]
        headlineVec = map(
            lambda x: reduce(np.add, [model[y] for y in x
                                      if y in model], [0.] * 300),
        ...
        print('headline word2vec features of training set saved in %s' %
              outfilename_hvec_train)
        ...
        if n_test > 0:
            ...
            print('headline word2vec features of test set saved in %s' %
                  outfilename_hvec_test)
        print('headine done')
        ...
        print('body word2vec features of training set saved in %s' %
              outfilename_bvec_train)
        ...
        if n_test > 0:
            ...
            print('body word2vec features of test set saved in %s' %
                  outfilename_bvec_test)

        print('body done')

        # compute cosine similarity between headline/body word2vec features
        simVec = np.asarray(map(cosine_sim, headlineVec,
                                bodyVec))[:, np.newaxis]
        print('simVec.shape:')
        print(simVec.shape)
        ...
        print('word2vec sim. features of training set saved in %s' %
              outfilename_simvec_train)
        ...
        if n_test > 0:
            ...
            print('word2vec sim. features of test set saved in %s' %
                  outfilename_simvec_test)
